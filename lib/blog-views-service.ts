...
export class BlogViewsService {
  ...
  static async getViewStats(blogPostId: string): Promise<BlogPostViewStats> {
    try {
      ...
      const uniqueViews = new Set(data?.map(view => view.viewer_id).filter(Boolean)).size;
      const lastViewAt = data?.length > 0 
  ...
  static async getBulkViewStats(blogPostIds: string[]): Promise<Record<string, BlogPostViewStats>> {
    try {
      ...
      if (data) {
        const groupedByPost = data.reduce((acc, view) => {
        ...
        Object.entries(groupedByPost).forEach(([postId, views]) => {
          ...
          const uniqueViews = new Set(views.map(v => v.viewer_id).filter(Boolean)).size;
          const lastViewAt = views.length > 0 
            ? views.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())[0].created_at
            : null;
