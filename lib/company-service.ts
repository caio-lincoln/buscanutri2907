...
export interface JobApplication {
  ...
  job_id: string
  candidate_id: string
  status: "pendente" | "em_analise" | "aprovado" | "rejeitado" | "entrevista" | "contratado"
  cover_letter?: string
  applied_at: string
  updated_at: string
...
export async function createJob(jobData: Omit<JobPosting, "id" | "applications_count" | "created_at" | "updated_at">) {
  const { data, error } = await supabase.from("job_postings").insert(jobData).select().single()
...
export async function updateJob(jobId: string, updates: Partial<JobPosting>) {
  const { data, error } = await supabase.from("job_postings").update(updates).eq("id", jobId).select().single()
...
export async function exportApplicationsToCSV(companyId: string): Promise<string> {
  try {
...
// PROCESSOS SELETIVOS
export async function getSelectionProcesses(companyId: string): Promise<SelectionProcess[]> {
  try {
    const applications = await getJobApplications(companyId)
    ...
    const enrichedProcesses = (processes || []).map((process) => {
      const application = applications.find((app) => app.id === process.application_id)
      return {
        ...process,
...
export async function createSelectionProcess(processData: {
  ...
  const { data, error } = await supabase.from("selection_processes").insert(processData).select().single()

  if (error) throw error
  return data as SelectionProcess
}

export async function updateSelectionProcess(processId: string, updates: Partial<SelectionProcess>) {
  const { data, error } = await supabase
    .from("selection_processes")
    .update(updates)
    .eq("id", processId)
...
export async function getCompanyStats(companyId: string): Promise<CompanyStats> {
  try {
    const [jobs, applications] = await Promise.all([getCompanyJobs(companyId), getJobApplications(companyId)])

    const totalJobs = jobs.length
    const activeJobs = jobs.filter((job) => job.status === "ativa").length
    const totalApplications = applications.length
    const hiredCandidates = applications.filter((app) => app.status === "contratado").length
    const conversionRate = totalApplications > 0 ? Math.round((hiredCandidates / totalApplications) * 100) : 0

    // Calcular tempo médio de contratação
    const hiredApplications = applications.filter((app) => app.status === "contratado")
    const avgHiringTime =
      hiredApplications.length > 0
        ? Math.round(
            hiredApplications.reduce((acc, app) => {
              const daysDiff = Math.floor(
                (new Date().getTime() - new Date(app.applied_at).getTime()) / (1000 * 60 * 60 * 24),
              )
              return acc + daysDiff
...
export async function getApplicationsReport(companyId: string, period: "7d" | "30d" | "90d" | "1y" = "30d") {
  try {
    ...
    const applications = await getJobApplications(companyId)
    const filteredApplications = applications.filter((app) => new Date(app.applied_at) >= startDate)

    // Agrupar por status
    const statusReport = {
      pendente: filteredApplications.filter((app) => app.status === "pendente").length,
      em_analise: filteredApplications.filter((app) => app.status === "em_analise").length,
      aprovado: filteredApplications.filter((app) => app.status === "aprovado").length,
      rejeitado: filteredApplications.filter((app) => app.status === "rejeitado").length,
      entrevista: filteredApplications.filter((app) => app.status === "entrevista").length,
      contratado: filteredApplications.filter((app) => app.status === "contratado").length,
    }
...
export async function updateCompanyProfile(companyId: string, updates: Partial<CompanyProfile>) {
  const { data, error } = await supabase
    .from("company_profiles")
    .update({
      ...updates,
...
export async function getCurrentUserProfile() {
  try {
    ...
    const { data: profile, error } = await supabase.from("company_profiles").select("*").eq("user_id", user.id).single()
