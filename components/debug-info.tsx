    ...
    try {
      ...
      setDebugInfo({
        ...
        tables: {
          ...
          nutritionist_profiles: { accessible: !nutritionistError, error: nutritionistError?.message },
          patient_profiles: { accessible: !patientError, error: patientError?.message },
          company_profiles: { accessible: !companyError, error: companyError?.message },
        },
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto mt-8">
      ...
      <CardContent className="space-y-4">
        ...
        {debugInfo && (
          <pre className="bg-gray-100 p-4 rounded-lg text-xs overflow-auto">{JSON.stringify(debugInfo, null, 2)}</pre>
        )}
      </CardContent>
    </Card>
  )
