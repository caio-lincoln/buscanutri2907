...
export function ProcessesTab() {
  ...
  const loadProcesses = useCallback(async () => {
    ...
    try {
      ...
      // Silent error handling: Error fetching company processes
    } finally {
      setLoading(false)
    }
  }, [user?.id])
  ...
  const statusStats = {
    ...
    successRate:
      processes.length > 0
        ? Math.round(
            (processes.filter(p => p.status === 'aprovado').length /
              processes.length) *
              100
          )
        : 0,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600">
            Acompanhe o andamento dos seus processos de recrutamento
          </p>
        </div>
        <Button
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">
            Não há processos seletivos que correspondam aos filtros aplicados.
          </p>
        </div>
      ) : (
