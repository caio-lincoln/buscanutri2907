...
import { MessageSquare, MessageCircle, Heart, ExternalLink, ThumbsUp, Eye, HelpCircle, Award, Plus } from "lucide-react"

import { type ForumQuestion, getAllForumQuestions, getNutritionistForumQuestions } from "@/lib/forum-data"
...
export function ForumTab() {
  ...
  useEffect(() => {
    ...
    const loadQuestions = async () => {
      ...
      const nutritionistQuestionsWithBadges = await getNutritionistForumQuestions()
    ...
    <div className="space-y-8">
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600">Acompanhe as discussões e contribua com a comunidade.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <Card className="border-0 shadow-lg backdrop-blur-sm">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{forumStats.totalQuestions}</div>
            <p className="text-xs text-gray-500">Perguntas feitas na comunidade</p>
          </CardContent>
        </Card>
        <Card className="border-0 shadow-lg backdrop-blur-sm">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Card className="border-0 shadow-lg backdrop-blur-sm">
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-500">Curtidas em perguntas e respostas</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="recent-questions" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="recent-questions">Perguntas de Paciente</TabsTrigger>
          <TabsTrigger value="nutritionist-questions">Perguntas de Nutricionistas</TabsTrigger>
        </TabsList>

        <TabsContent value="recent-questions" className="mt-6">
          <h2 className="text-2xl font-bold text-[#1E1D40] mb-6">Perguntas de Pacientes</h2>
          <div className="space-y-6">
              ...
              (
                ...
                <Card key={question.id} className="border-0 shadow-lg backdrop-blur-sm">
                  ...
                  <CardContent className="space-y-4">
                    <Link href={`/dashboard/nutricionistas/forum/${question.id}`} className="block">
                    ...
                    <div className="flex items-center gap-2 text-sm text-gray-600">
                      ...
                      <span>•</span>
                      <span className="flex items-center gap-1">
                        <MessageCircle className="h-4 w-4" /> {question.repliesCount}
                      </span>
                      <span>•</span>
                      <span className="flex items-center gap-1">
            ...
            {questions.length === 0 && (
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-semibold text-[#1E1D40] mb-2">Nenhuma pergunta de paciente</h3>
                <p className="text-gray-600 mb-6">Não há perguntas de pacientes no momento.</p>
              </div>
            )}
          </div>
        </TabsContent>
        ...
        <TabsContent value="nutritionist-questions" className="mt-6">
          <h2 className="text-2xl font-bold text-[#1E1D40] mb-6">Perguntas de Nutricionistas</h2>
          <div className="space-y-6">
            ...
            {nutritionistQuestions.length === 0 && (
              <div className="text-center py-12">
                <HelpCircle className="h-12 w-12 text-gray-400 mx-auto mb-4" />
                <h3 className="text-xl font-semibold text-[#1E1D40] mb-2">Nenhuma pergunta de nutricionista</h3>
                <p className="text-gray-600 mb-6">Não há perguntas de nutricionistas no momento.</p>
              </div>
            )}
          </div>
        </TabsContent>
