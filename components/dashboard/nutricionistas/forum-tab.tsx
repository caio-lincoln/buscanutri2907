...
import { MessageSquare, MessageCircle, Heart, ExternalLink, ThumbsUp, Eye, HelpCircle, Award } from "lucide-react"

import { type ForumQuestion, getAllForumQuestions } from "@/lib/forum-data"
    ...
    <div className="space-y-8">
      ...
      <Tabs defaultValue="recent-questions" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="recent-questions">Perguntas Recentes</TabsTrigger>
          <TabsTrigger value="popular-questions">Perguntas Populares</TabsTrigger>
        </TabsList>

        <TabsContent value="recent-questions" className="mt-6">
          <h2 className="text-2xl font-bold text-[#1E1D40] mb-6">Últimas Perguntas</h2>
          <div className="space-y-6">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-600">
                      <div className="flex items-center gap-1">
                        <ThumbsUp className="h-4 w-4" /> {question.likes} Curtidas
                      </div>
                      <div className="flex items-center gap-1">
            ...
            {questions.length === 0 && (
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-semibold text-[#1E1D40] mb-2">Nenhuma pergunta recente</h3>
                <p className="text-gray-600 mb-6">Não há perguntas recentes no momento.</p>
              </div>
            )}
          </div>
        </TabsContent>

        <TabsContent value="popular-questions" className="mt-6">
          <h2 className="text-2xl font-bold text-[#1E1D40] mb-6">Perguntas Populares</h2>
          <div className="space-y-6">
            {questions
              .slice()
              ...
              .map(
                (
                  question, // Show top 3 popular questions
                ) => (
                  <Card key={question.id} className="border-0 shadow-lg backdrop-blur-sm">
                    <CardHeader className="flex flex-row items-center justify-between pb-3">
                      <div className="flex items-center gap-3">
            ...
            {questions.length === 0 && (
              <div className="text-center py-12">
                <HelpCircle className="h-12 w-12 text-gray-400 mx-auto mb-4" />
                <h3 className="text-xl font-semibold text-[#1E1D40] mb-2">Nenhuma pergunta popular</h3>
                <p className="text-gray-600 mb-6">Não há perguntas populares no momento.</p>
              </div>
            )}
          </div>
        </TabsContent>
