...
export function JobsTab() {
  ...
  const [filterStatus, setFilterStatus] = useState<JobData['status'] | 'all'>(
    'all'
  )
  ...
  const handleAction = (action: string, job: JobData) => {
    ...
    alert(
      `Ação: ${action} para vaga "${job.title}" da empresa "${job.company}"`
    )
  }
        ...
        <CardHeader className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pb-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            ...
            <Select
              value={filterStatus}
              onValueChange={value =>
                setFilterStatus(value as JobData['status'] | 'all')
              }
            >
