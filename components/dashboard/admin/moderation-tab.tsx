...
export function ModerationTab() {
  ...
  const [filterType, setFilterType] = useState<ReportData['type'] | 'all'>(
    'all'
  )
  const [filterStatus, setFilterStatus] = useState<
    ReportData['status'] | 'all'
  >('all')
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-3xl font-bold text-[#1E1D40]">
        Moderação de Conteúdo e Usuários
      </h2>
        ...
        <CardHeader className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pb-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            ...
            <Select
              value={filterType}
              onValueChange={value =>
                setFilterType(value as ReportData['type'] | 'all')
              }
            >
            ...
            <Select
              value={filterStatus}
              onValueChange={value =>
                setFilterStatus(value as ReportData['status'] | 'all')
              }
            >
              <SelectTrigger className="w-full sm:w-[180px] rounded-lg border border-gray-200 focus:ring-emerald-500 focus:border-emerald-500">
                <Filter className="h-4 w-4 mr-2 text-gray-500" />
                  ...
                  currentReports.map(report => {
                    ...
                    return (
                      <TableRow key={report.id} className="hover:bg-gray-50">
                        ...
                        <TableCell>
                          <Badge
                            className={`capitalize ${reportStatusColors[report.status]}`}
                          >
                            {report.status}
                          </Badge>
                        </TableCell>
