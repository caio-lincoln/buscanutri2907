...
async function setupBadgesTables() {
  ...
  try {
    ...
    const { data: nutritionistBadges, error: nutritionistBadgesError } = await supabase
    ...
    if (!nutritionistBadgesError) {
      ...
      console.log('⚠️ Tabela nutritionist_badges não encontrada:', nutritionistBadgesError.message);
    }
    ...
    if (badgesError || nutritionistBadgesError) {
      ...
      console.log('3. Execute o conteúdo do arquivo: scripts/create-badges-tables.sql');
  ...
  UNIQUE (nutritionist_id, badge_id)
      ...
      console.log('ALTER TABLE public.nutritionist_badges ENABLE ROW LEVEL SECURITY;');
      console.log('\n-- Criar políticas');
      console.log('CREATE POLICY "Enable read access for all users" ON public.badges FOR SELECT USING (true);');
      console.log('CREATE POLICY "Enable read access for all users" ON public.nutritionist_badges FOR SELECT USING (true);');
      return;
    }
      ...
      const sampleBadges = [
        {
          ...
          icon_url: 'https://cdn.jsdelivr.net/npm/lucide@latest/icons/shield-check.svg'
        },
        {
          name: 'Especialista em Esportes',
          description: 'Especialista em nutrição esportiva',
          icon_url: 'https://cdn.jsdelivr.net/npm/lucide@latest/icons/trophy.svg'
        },
        {
          name: 'Top Avaliado',
        ...
        },
        {
          ...
          icon_url: 'https://cdn.jsdelivr.net/npm/lucide@latest/icons/video.svg'
        }
      ];
