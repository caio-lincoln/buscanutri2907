...
async function testConnection() {
  try {
    ...
    if (error) {
      console.log('⚠️  Sessão de auth não disponível, mas isso é normal para service role');
    }
    ...
    if (testError) {
      console.log('⚠️  Não foi possível acessar auth.users diretamente, tentando patient_profiles...');
      ...
      if (patientError) {
        console.error('❌ Erro ao acessar patient_profiles:', patientError.message);
        return false;
      } else {
        console.log('✅ Conexão estabelecida! Tabela patient_profiles acessível');
      ...
      if (patientError) {
        console.error('❌ Erro ao acessar patient_profiles:', patientError.message);
        console.log('💡 Isso pode indicar problemas com RLS ou estrutura da tabela');
      } else {
...
async function main() {
  ...
  if (success) {
    ...
    console.log('💡 O problema pode estar na configuração do MCP, não nas credenciais.');
  } else {
