...
async function addCancellationPolicyColumn() {
  try {
    ...
    try {
      ...
      if (funcError) {
        console.log(
          '⚠️  Função exec_sql pode não existir, tentando abordagem alternativa...'
        )
      } else {
    ...
    try {
      ...
      console.log(
        '⚠️  RPC não funcionou, a coluna precisa ser adicionada manualmente'
      )
      console.log('')
    ...
    if (error) {
      ...
      console.log(
        '🎉 O erro de atualização do perfil de nutricionista foi corrigido!'
      )
    }
  } catch (error) {
