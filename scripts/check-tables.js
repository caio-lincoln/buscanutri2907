...
if (!supabaseUrl || !supabaseKey) {
  console.error(
    'Erro: Variáveis de ambiente NEXT_PUBLIC_SUPABASE_URL e NEXT_PUBLIC_SUPABASE_ANON_KEY são necessárias'
  )
  process.exit(1)
}
...
async function checkTables() {
  try {
    ...
    if (ratingsError) {
      console.error(
        'Erro ao verificar tabela consultation_ratings:',
        ratingsError
      )
    } else {
    ...
    if (reviewsError) {
      console.error(
        'Erro ao verificar tabela consultation_reviews:',
        reviewsError
      )
    } else {
