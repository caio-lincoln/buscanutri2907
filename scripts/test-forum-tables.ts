...
async function testForumTables() {
  try {
    ...
    if (questionsError) {
      ...
      console.log('✅ forum_questions OK - Registros encontrados:', questions?.length || 0)
    ...
    if (answersError) {
      ...
      console.log('✅ forum_answers OK - Registros encontrados:', answers?.length || 0)
    ...
    if (answerLikesError) {
      ...
      console.log('✅ forum_answer_likes OK - Registros encontrados:', answerLikes?.length || 0)
    }
    ...
    if (questionLikesError) {
      console.log('❌ Erro em forum_question_likes:', questionLikesError.message)
    } else {
      console.log('✅ forum_question_likes OK - Registros encontrados:', questionLikes?.length || 0)
    }
      ...
      if (users?.users && users.users.length > 0) {
        ...
        const { data: testQuestion, error: insertError } = await supabase
          ...
          .insert({
            ...
            content: 'Esta é uma pergunta de teste para verificar se a inserção funciona.',
        ...
        if (insertError) {
          console.log('❌ Erro ao inserir pergunta de teste:', insertError.message)
        } else {
          console.log('✅ Pergunta de teste inserida com sucesso:', testQuestion?.id)
