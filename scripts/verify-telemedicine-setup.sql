...
SELECT 
    'telemedicine_consultations' as table_name,
...
SELECT 
    ...
    CASE 
        WHEN c.start_time > NOW() THEN 'Futura'
        WHEN c.start_time > NOW() - INTERVAL '1 hour' THEN 'Disponível'
        ELSE 'Expirada'
    END as availability
...
AND tablename IN ('telemedicine_consultations', 'consultation_messages', 'consultation_notes', 'consultation_sessions', 'webrtc_signals')
ORDER BY tablename;
...
FROM pg_indexes 
WHERE schemaname = 'public' 
AND tablename IN ('telemedicine_consultations', 'consultation_messages', 'consultation_notes')
