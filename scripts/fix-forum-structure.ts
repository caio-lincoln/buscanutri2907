  ...
  try {
    ...
    try {
      ...
      if (addAuthorIdError && !addAuthorIdError.message.includes('exec_sql')) {
        ...
        console.log('✅ Coluna author_id adicionada/verificada em forum_questions')
      }
    } catch (error) {
    ...
    try {
      ...
      if (addAnswerAuthorIdError && !addAnswerAuthorIdError.message.includes('exec_sql')) {
        console.log('❌ Erro ao adicionar author_id:', addAnswerAuthorIdError.message)
      } else {
        console.log('✅ Coluna author_id adicionada/verificada em forum_answers')
      }
    } catch (error) {
      console.log('⚠️  Tentativa alternativa para adicionar author_id...')
    }
    ...
    try {
      ...
      if (createTableError && !createTableError.message.includes('exec_sql')) {
        console.log('❌ Erro ao criar forum_question_likes:', createTableError.message)
      } else {
    ...
    if (questionLikesError) {
      console.log('❌ forum_question_likes ainda com erro:', questionLikesError.message)
    } else {
