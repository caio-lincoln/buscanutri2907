...
async function applyMigration() {
  try {
    ...
    // Ler o arquivo SQL
    const sqlPath = path.join(__dirname, '..', 'supabase', 'migrations', '20240801000001_create_consultation_ratings.sql');
      ...
      if (command.trim()) {
        try {
          ...
          if (error) {
            ...
            const { error: queryError } = await supabase.from('_exec_sql').select('*').eq('sql', command + ';');
            
            if (queryError) {
              console.error(`❌ Erro ao executar com query:`, queryError.message);
              // Tentar com outro método
              console.log(`⏳ Tentando executar com query SQL direta...`);
              const { error: directError } = await supabase.auth.admin.executeSql(command + ';');
              
              if (directError) {
                console.error(`❌ Erro ao executar diretamente:`, directError.message);
              } else {
                console.log(`✅ Comando ${i + 1} executado com sucesso via SQL direto`);
              }
            } else {
