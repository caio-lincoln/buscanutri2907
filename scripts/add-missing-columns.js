...
async function addMissingColumns() {
  console.log('🔧 Iniciando adição de colunas faltantes na tabela nutritionist_profiles...\n')
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('exec_sql')) {
          ...
          if (columnsError) {
            console.error(`❌ Erro ao verificar coluna ${column.name}:`, columnsError.message)
          } else if (columns && columns.length > 0) {
            console.log(`✅ Coluna ${column.name} já existe`)
          } else {
            console.log(`❌ Coluna ${column.name} não existe e não pode ser criada automaticamente`)
            console.log(`   Execute manualmente: ALTER TABLE public.nutritionist_profiles ADD COLUMN IF NOT EXISTS ${column.name} ${column.type};`)
          }
        }
      } else {
      ...
      console.error(`❌ Erro inesperado ao adicionar coluna ${column.name}:`, err.message)
    }
  }
  ...
  try {
    ...
    if (tableError) {
      console.error('❌ Erro ao consultar estrutura da tabela:', tableError.message)
    } else {
  ...
  console.log('\n✅ Script concluído!')
  console.log('\n📝 Próximos passos:')
  console.log('1. Se as colunas foram adicionadas com sucesso, remova a solução temporária do profile-service.ts')
  console.log('2. Se houve erro, execute os comandos SQL manualmente no dashboard do Supabase')
  console.log('3. Teste a atualização de perfil de nutricionista para confirmar que funciona')
}
