...
function testRemovalLogic() {
  ...
  if (
    ...
    dataToUpdate.cancellation_policy !== undefined
  ) {
    console.warn(
      '⚠️ Campo cancellation_policy removido temporariamente - coluna não existe no banco'
    )
  ...
  if (dataToUpdate.cancellation_policy === undefined) {
    console.log(
      '✅ Sucesso! O campo cancellation_policy foi removido corretamente'
    )
    console.log(
      '🎉 O erro de "Could not find the cancellation_policy column" não deve mais ocorrer'
    )
  } else {
