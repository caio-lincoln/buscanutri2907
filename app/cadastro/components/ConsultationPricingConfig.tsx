...
export default function ConsultationPricingConfig({ 
  ...
  const validatePricing = (): string[] => {
    ...
    if (!pricingConfig.inPerson.enabled && !pricingConfig.online.enabled) {
      validationErrors.push('Pelo menos uma modalidade (Presencial ou Online) deve estar configurada.');
    }
    ...
    if (pricingConfig.inPerson.enabled) {
      if (pricingConfig.inPerson.pricingType === 'combined') {
        ...
        if (price === null || price < 0) {
          validationErrors.push('Preço único presencial deve ser maior ou igual a R$ 0,00.');
        }
      } else if (pricingConfig.inPerson.pricingType === 'separate') {
        const consultationPrice = parseCurrency(pricingConfig.inPerson.consultationPrice);
        const followupPrice = parseCurrency(pricingConfig.inPerson.followupPrice);
        
        if (consultationPrice === null || consultationPrice < 0) {
          validationErrors.push('Preço da consulta presencial deve ser maior ou igual a R$ 0,00.');
        }
        if (followupPrice === null || followupPrice < 0) {
          validationErrors.push('Preço do retorno presencial deve ser maior ou igual a R$ 0,00.');
        }
      }
    }
    ...
    if (pricingConfig.online.enabled) {
      if (pricingConfig.online.pricingType === 'combined') {
        ...
        if (price === null || price < 0) {
          validationErrors.push('Preço único online deve ser maior ou igual a R$ 0,00.');
        }
      } else if (pricingConfig.online.pricingType === 'separate') {
        const consultationPrice = parseCurrency(pricingConfig.online.consultationPrice);
        const followupPrice = parseCurrency(pricingConfig.online.followupPrice);
        
        if (consultationPrice === null || consultationPrice < 0) {
          validationErrors.push('Preço da consulta online deve ser maior ou igual a R$ 0,00.');
        }
        if (followupPrice === null || followupPrice < 0) {
          validationErrors.push('Preço do retorno online deve ser maior ou igual a R$ 0,00.');
        }
      }
    }
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-6">
        ...
        <Alert>
          ...
          <AlertDescription>
            Configure os preços para suas consultas. Você pode escolher entre preço único (consulta + retorno) 
            ou preços separados para cada modalidade. É obrigatório configurar pelo menos uma modalidade.
          </AlertDescription>
        </Alert>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="in-person-enabled">Ofereço consultas presenciais</Label>
            </div>
          </div>
            ...
            <>
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <Label htmlFor="in-person-combined">Preço único (consulta + retorno)</Label>
                </div>
                <div className="flex items-center space-x-2">
            ...
            <>
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <Label htmlFor="online-combined">Preço único (consulta + retorno)</Label>
                </div>
                <div className="flex items-center space-x-2">
        ...
        {(pricingConfig.inPerson.enabled || pricingConfig.online.enabled) && (
          <div className="bg-gray-50 p-4 rounded-lg">
            <h4 className="font-semibold mb-2">Resumo dos Preços Configurados:</h4>
            <div className="space-y-2 text-sm">
              {pricingConfig.inPerson.enabled && (
                <div>
                  <strong>Presencial:</strong>{' '}
                  {pricingConfig.inPerson.pricingType === 'combined' && pricingConfig.inPerson.combinedPrice
              ...
              {pricingConfig.online.enabled && (
                <div>
                  ...
                  {pricingConfig.online.pricingType === 'combined' && pricingConfig.online.combinedPrice
