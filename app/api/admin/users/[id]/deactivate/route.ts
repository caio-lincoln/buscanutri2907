...
export const POST = withErrorHandling(async (req: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
  ...
  const banDuration: string = typeof duration === 'string' && duration.length > 0 ? duration : '720h'

  const { error } = await admin.auth.admin.updateUserById(targetUuid, { ban_duration: banDuration })
  if (error) {
    const msg = error.message || 'Falha ao desativar usuário'
