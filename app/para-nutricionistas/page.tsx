...
const benefits = [
  {
    ...
    description: "Conecte-se com milhares de pessoas que buscam acompanhamento nutricional qualificado.",
    stats: "+300% pacientes",
  },
  {
    icon: Calendar,
    title: "Gestão Inteligente",
    description: "Sistema completo de agendamento, prontuários digitais e acompanhamento de pacientes.",
    stats: "100% digital",
  },
  {
    icon: TrendingUp,
    title: "Crescimento Profissional",
    description: "Acesso a cursos, eventos e networking com outros profissionais da área.",
    stats: "+50 cursos",
  },
  {
    icon: Shield,
    title: "Credibilidade",
    description: "Perfil verificado que transmite confiança e profissionalismo aos seus pacientes.",
    stats: "Selo verificado",
  },
  {
    icon: Zap,
    title: "Tecnologia Avançada",
    description: "Ferramentas modernas para consultas online, chat com pacientes e relatórios detalhados.",
    stats: "IA integrada",
  },
  {
    icon: Award,
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-cyan-50">
      ...
      <main>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 pt-8">
                  {stats.map((stat, index) => (
                    <div key={index} className="text-center">
                      <div className="text-2xl font-bold text-[#4AB0D9]">{stat.number}</div>
                      <div className="text-sm text-[#1E1D40]/60">{stat.label}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="container px-4 md:px-6">
            <div className="text-center mb-16">
              <h2 className="text-3xl md:text-5xl font-bold text-[#1E1D40] mb-6">
                Por que escolher a <span className="text-[#4AB0D9]">Busca Nutri</span>?
              </h2>
              <p className="text-xl text-[#1E1D40]/70 max-w-3xl mx-auto">
                A plataforma completa para nutricionistas que querem crescer e se destacar no mercado
              </p>
            </div>

            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {benefits.map((benefit, index) => (
                <Card key={index} className="border border-blue-100 hover:shadow-lg transition-shadow">
                  <CardContent className="p-8 text-center">
                    ...
                    <p className="text-[#1E1D40]/70 mb-4">{benefit.description}</p>
                    <Badge variant="secondary" className="bg-[#4AB0D9]/10 text-[#4AB0D9]">
                      {benefit.stats}
                    </Badge>
                  </CardContent>
                </Card>
        ...
        <section className="py-20 bg-white">
          <div className="container px-4 md:px-6">
            <div className="text-center mb-16">
              <h2 className="text-3xl md:text-4xl font-bold text-[#1E1D40] mb-6">Planos que Crescem com Você</h2>
              <p className="text-lg text-[#1E1D40]/70">Escolha o plano ideal para o seu momento profissional</p>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
              {plans.map((plan, index) => (
                <Card
                  key={index}
                  className={`relative ${
                    plan.popular ? "border-2 border-[#4AB0D9] shadow-lg scale-105" : "border border-gray-200"
                  }`}
                >
                  {plan.popular && (
                    <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
                      <Badge className="bg-[#4AB0D9] text-white px-4 py-1">Mais Popular</Badge>
                    </div>
                  )}
                  ...
                  <CardContent>
                    ...
                    >
                      {plan.price === "Gratuito" ? "Começar Grátis" : "Escolher Plano"}
                    </Button>
                  </CardContent>
                </Card>
              ))}
          ...
          <div className="container px-4 md:px-6 text-center">
            <div className="max-w-3xl mx-auto space-y-8">
              <h2 className="text-3xl md:text-5xl font-bold">
                Pronto para <span className="text-blue-200">expandir</span> sua prática?
              </h2>
              <p className="text-xl text-white/90">
                Junte-se a centenas de nutricionistas que já transformaram suas carreiras com a Busca Nutri
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link href="/cadastro?tipo=nutricionista">
                  <Button size="lg" variant="secondary" className="bg-white text-[#4AB0D9] hover:bg-gray-100">
                    Começar Gratuitamente
                    <ArrowRight className="ml-2 h-5 w-5" />
                  </Button>
                </Link>
              ...
              <p className="text-sm text-white/70">Sem compromisso • Cancele quando quiser • Suporte especializado</p>
            </div>
          </div>
        </section>
      </main>
