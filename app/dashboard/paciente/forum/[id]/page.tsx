...
export default function PatientForumQuestionPage() {
  ...
  const applyFiltersAndSort = (replies: any[], searchTerm: string, sortOrder: string) => {
    ...
    switch (sortOrder) {
      case "recent":
        filteredReplies.sort((a, b) => new Date(b.timestamp || 0).getTime() - new Date(a.timestamp || 0).getTime())
        break
      case "likes":
  ...
  const handleSubmitReply = async () => {
    ...
    try {
      const newReply = await createForumAnswer(questionId, replyContent, currentUser.id)
      if (newReply) {
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">{new Date(question.timestamp).toLocaleDateString('pt-BR')}</p>
              </div>
            </div>
        ...
        <CardContent className="space-y-4">
          <h1 className="text-2xl font-bold text-[#1E1D40]">{question.title}</h1>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {reply.author?.userType === 'nutricionista' && reply.author?.isVerified && (
                          <Award className="h-4 w-4 text-yellow-500" />
                        )}
                        {reply.isBestAnswer && (
                      ...
                      <p className="text-sm text-gray-500">{new Date(reply.timestamp).toLocaleDateString('pt-BR')}</p>
                    </div>
                  </div>
                </div>
              </CardHeader>
        ...
        {currentUser && (
          <Card>
            <CardHeader>
              <h3 className="text-lg font-semibold text-[#1E1D40]">Sua Resposta</h3>
            </CardHeader>
            <CardContent className="space-y-4">
              <Textarea
                placeholder={
              ...
              <div className="flex justify-end">
                <Button
                  ...
                  disabled={!replyContent.trim() || isSubmittingReply || currentUser.user_type !== 'nutritionist'}
