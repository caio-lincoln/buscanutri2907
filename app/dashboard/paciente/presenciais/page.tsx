...
export default function PresenciaisPage() {
  ...
  const loadAppointments = useCallback(async () => {
    ...
    try {
      ...
      // NOTE: Using 'appointments' table as requested for presencial flow.
      // Filtering by type='presencial' to ensure isolation.
      let q = supabase
        .from('appointments')
        .select(`
        ...
        .eq('type', 'presencial') // Only presencial
        .order('appointment_date', { ascending: false })
        .order('appointment_time', { ascending: false })

      const { status, dateFrom, dateTo } = filters
      if (status && status !== 'all') q = q.eq('status', status)
      if (dateFrom) q = q.gte('appointment_date', format(dateFrom, 'yyyy-MM-dd'))
      if (dateTo) q = q.lte('appointment_date', format(dateTo, 'yyyy-MM-dd'))
      ...
      const mapped = ((data as any[]) || []).map((row: any) => ({
        id: row.id,
        appointment_date: `${row.appointment_date}T${row.appointment_time}`, // Construct ISO for display
        ...
        address_main: row.address, // Use the joined address
